...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if transition == "S":
            ...
            self.first_elem = self.stack.pop()
            self.second_elem = self.stack.pop()
            self.dependencies.append((self.first_elem, self.second_elem))
            self.stack.append(self.first_elem)

        elif transition == "RA":
            self.first_elem = self.stack.pop()
            self.second_elem = self.stack[-1]
            self.dependencies.append((self.second_elem, self.first_elem))
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses) != 0:
        ...
        for parse, prediction in zip(minibatch, batch_transitions):
